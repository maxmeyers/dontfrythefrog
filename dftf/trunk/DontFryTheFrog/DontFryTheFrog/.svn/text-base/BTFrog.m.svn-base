...
@synthesize wanderAction, preCatchSequence, catchSequence, tongueSprite, foilHat;
...
@synthesize tongueTip, inCatchSequence, inTransition, inRespawn, currentBug;
...
-(id) initWithFile:(NSString *) fileName IdleAnimation:(CCAnimation *) animation ParentLayer:layer{
    ...
    if ( (self=[super initWithFile:fileName Position:CGPointMake(440, 80) IdleAnimation:animation ParentLayer:layer])) { 
        ...
        self.foilHat = [CCSprite spriteWithSpriteFrameName:@"foil-hat.png"];
        [self addChild:foilHat z:3 tag:kFoilHatSpriteTag];

        
        NSString *currentHat = [BTStorageManager currentHat];
        if (![currentHat isEqualToString:@""]){
            NSDictionary *hatDetails = [[[[BTHopshopManager sharedHopshopManager] hopshopDetails] hats] objectForKey:currentHat];
            if (hatDetails != NULL && [hatDetails objectForKey:@"filename"] != NULL){
                CCSprite *hat = [CCSprite spriteWithSpriteFrameName:[hatDetails objectForKey:@"filename"]];
                [self addChild:hat z:3 tag:kHatTag];      
            }           
        }
...
//    self.position = ccp(999,999); // if you really want to get rid of the frog
    ...
    if ([[self children] containsObject:foilHat]){
        foilHat.scale = 1/self.scale;
    }
    
    if ([self getChildByTag:kHatTag]){
        // So, this class is already fucked to hell, so I'm just gonna go ahead and do this.
        if (fatLevel == 2){
            [[self getChildByTag:kHatTag] setScale:1/[self frogScale]];            
        } else {
            [[self getChildByTag:kHatTag] setScale:1];
        }
    }
}
...
- (void) reset {
    ...
    [[self currentAnimations] jawSprite].visible = NO;
    [self setFatLevel:2];
    ...
    foilHat.position = ccp(0, 46);
    [self addChild:foilHat z:3];
...
-(void) eat:(id)node data:(BTBug *)bug {
    ...
    if (![bug dead] && [bug UUID] == bugUUID){
        ...
        [self setInCatchSequence:NO];
        if (currentEffect == nil || [currentEffect isMemberOfClass:[BTFrogEffectFoilHat class]]){
            [self runIdleAction];
        }
    }
    lastCatch = [gameLayer playTime];    
...
- (void) finishedEating:(id) node data:(BTBug *)bug {
    ...
    if ([[bug class] isSubclassOfClass:[BTGoodBug class]]) {
        [[self gameLayer] showPointsFadeawayString:@"50" position:self.position];
        [[[self gameLayer] scene] addPoints:50];        
    }
}
