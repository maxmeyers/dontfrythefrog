...
- (id) init {
    ...
    if (self){
        ...
        hopshopMenuItem = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"hopshop-unpressed.png"] selectedSprite:[CCSprite spriteWithSpriteFrameName:@"hopshop-pressed.png"] disabledSprite:[CCSprite spriteWithSpriteFrameName:@"hopshop-pressed.png"] block:^(id sender){
                if ([BTStorageManager levelForTag:@"EverShoppedFromResults"] == 0){
                    [BTStorageManager setLevel:1 ForTag:@"EverShoppedFromResults"];
                    [FlurryAnalytics logEvent:@"Opened Hopshop From Results for First Time"];         
                }
                [FlurryAnalytics logEvent:@"Opened Hopshop From Results"];
        ...
        twitterMenuItem = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"twitter.png"] selectedSprite:[CCSprite spriteWithSpriteFrameName:@"twitter-pressed.png"] target:self selector:@selector(twitterButtonPressed)];
        twitterMenuItem.position = ccp(112, 50);
        
        facebookMenuItem = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"facebook.png"] selectedSprite:[CCSprite spriteWithSpriteFrameName:@"facebook-pressed.png"] target:self selector:@selector(facebookButtonPressed)];
        facebookMenuItem.position = ccp(twitterMenuItem.position.x+44, twitterMenuItem.position.y);
        
        gcMenuItem = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"GameCenter.png"] selectedSprite:[CCSprite spriteWithSpriteFrameName:@"gamecenter-pressed.png"] target:self selector:@selector(gcButtonpressed)];
...
- (id) initWithScore:(int) score AllTimeScore:(int) allTimeScore FliesFried:(int) fliesFried AllTimeFliesFried:(int) allTimeFliesFried FlyPennies:(int) flyPennies Multiplier:(int) multiplier AllTimeMultiplier:(int) alltimeMultiplier Bank:(int) bank PlayTime:(ccTime) playTime {
    ...
    if (self){
        ...
        CCLabelTTF *bankLabel = [CCLabelTTF labelWithString:[numberFormatter stringFromNumber:[NSNumber numberWithInt:bank]] dimensions:CGSizeMake(100, 30) alignment:UITextAlignmentRight fontName:@"soupofjustice" fontSize:16];
        bankLabel.position = ccp(368, 168);
        ...
        BTDrTimBox *drTimBox = [[[BTDrTimBox alloc] initWithLastScore:score PlayTime:playTime AllTimeHighScoreBox:alltimeHighScoreBox WeeklyHighScoreBox:weeklyHighScoreBox] autorelease];
