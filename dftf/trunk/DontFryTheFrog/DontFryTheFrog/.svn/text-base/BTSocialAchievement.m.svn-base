...
+ (NSString *) stringForScore:(GKScore *)score {
    return [BTSocialAchievement stringForInt:score.value];
}

+ (NSString *) stringForInt:(int) number {
    NSNumberFormatter *numberFormatter = [NSNumberFormatter new];
    [numberFormatter setNumberStyle:NSNumberFormatterDecimalStyle];
    NSString *scoreString = [numberFormatter stringFromNumber:[NSNumber numberWithInt:number]];
    ...
    if ([GKLocalPlayer localPlayer].authenticated){
        ...
        if (myIndex != -1 && friendIndex != -1){
            ...
            if (previousScore < friendScore.value && score == myScore.value){
                ...
                NSString *scoreString = [BTSocialAchievement stringForScore:myScore];
                self.string = [NSString stringWithFormat:@"You beat %@'s %@ High Score with your score of %@!", [BTGameCenterManager aliasForPlayerID:friendScore.playerID Players:players], timePeriod, scoreString];
                self.socialString = [NSString stringWithFormat:@"I beat my friend %@'s High Score in Don't Fry the Frog with %@!", [BTGameCenterManager aliasForPlayerID:friendScore.playerID Players:players], scoreString];
                return YES; 
            }
        }
...
- (id) initWithTimePlayed:(int) timePlayed Score:(int) score PreviousAllTimeHighScore:(int) previousAllTimeHighScore PreviousWeeklyHighScore:(int) previousWeeklyHighScore FliesFried:(int) fliesFried PreviousHighFliesFried:(int) previousHighFliesFried AllTimeScores:(NSArray *) allTimeScores AllTimePlayers:(NSArray *) allTimePlayers WeeklyScores:(NSArray *) weeklyScores WeeklyPlayers:(NSArray *) weeklyPlayers {
    ...
    if (self){
        ...
        if (_achieved) {
            NSString *scoreString = [BTSocialAchievement stringForInt:score];
            self.string = [NSString stringWithFormat:@"You set a new All-Time Personal Best Score of %@!", scoreString];
            self.socialString = [NSString stringWithFormat:@"I beat my All-Time Best Score in Don't Fry the Frog with %@!", scoreString];
        }
    }
...
- (id) initWithTimePlayed:(int) timePlayed Score:(int) score PreviousAllTimeHighScore:(int) previousAllTimeHighScore PreviousWeeklyHighScore:(int) previousWeeklyHighScore FliesFried:(int) fliesFried PreviousHighFliesFried:(int) previousHighFliesFried AllTimeScores:(NSArray *) allTimeScores AllTimePlayers:(NSArray *) allTimePlayers WeeklyScores:(NSArray *) weeklyScores WeeklyPlayers:(NSArray *) weeklyPlayers {
    ...
    if (self){
        ...
        if (_achieved) {
            NSString *scoreString = [BTSocialAchievement stringForInt:score];
            self.string = [NSString stringWithFormat:@"You set a new Weekly Personal Best Score of %@!", scoreString];
            self.socialString = [NSString stringWithFormat:@"I beat my Weekly Best Score in Don't Fry the Frog with %@!", scoreString];
        }
    }
...
- (id) initWithScore:(int)score HighScore:(int)highScore FliesFried:(int)fliesFried HighFliesFried:(int)highFliesFried TimePlayed:(int)playTime {
    ...
    if (self){
        if (score > highScore){
            _achieved = YES;
            NSString *scoreString = [BTSocialAchievement stringForInt:score];
            self.string = [NSString stringWithFormat:@"You set a new Best Score of %@!", scoreString];
            self.socialString = [NSString stringWithFormat:@"I beat my Best Score in Don't Fry the Frog with %@!", scoreString];
        }
    }
    return self;
}
...
- (id) initWithScore:(int)score HighScore:(int)highScore FliesFried:(int)fliesFried HighFliesFried:(int)highFliesFried TimePlayed:(int)playTime {
    ...
    if (self){
        if (fliesFried > highFliesFried){
            ...
            self.socialString = [NSString stringWithFormat:@"I just scored %@ in Don't Fry the Frog!", scoreString];
        }
    }
    return self;
}
...
- (id) initWithScore:(int)score HighScore:(int)highScore FliesFried:(int)fliesFried HighFliesFried:(int)highFliesFried TimePlayed:(int)playTime {
    ...
    if (self){
        ...
        self.socialString = [NSString stringWithFormat:@"I just kept the frog alive for %d seconds in Don't fry the Frog!", playTime];
    }
    return self;
}
