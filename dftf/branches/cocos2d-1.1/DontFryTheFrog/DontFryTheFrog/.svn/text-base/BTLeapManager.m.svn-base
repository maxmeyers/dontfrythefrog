...
- (void) loadLevels {
    if ([[BTConfig sharedConfig] readFromPlist]){
        ...
        levels = (NSArray *)[[NSKeyedUnarchiver unarchiveObjectWithFile:@"Levels.bin"] retain];
    }
}
...
- (void) loadLeaps {
    if ([[BTConfig sharedConfig] readFromPlist]){
        ...
        leaps = [[NSKeyedUnarchiver unarchiveObjectWithFile:@"Leaps.bin"] retain];
    }
}
    ...
    while ([currentLeaps count] < 3){
        ...
        if (true){
            ...
            if ([currentLeaps count] == 0){
                newLeap = (BTLeap *)[(NSDictionary *)[leaps objectForKey:@"BTAtLeastGameLengthLeap"] objectForKey:@"withoutgrey30"];
            } else {
                newLeap = [self newLeap];
            } 
        }
...
- (void) notifyCompletionOfLeap:(BTLeap *) leap {
    ...
    if (notificaitonParent){        
        ...
        if (index != NSNotFound && index < 3){
            CCSprite *notification = [CCSprite spriteWithFile:[NSString stringWithFormat:@"LM%dcomplete.png", index+1]];
...
//                if (![BTStorageManager hasAchievedLeap:[leap name]] && [leap reward] < minimumLevelForCategory){
...
//                if ([leap reward] <= minimumLevelForCategory && ![BTStorageManager hasAchievedLeap:[leap name]]){
