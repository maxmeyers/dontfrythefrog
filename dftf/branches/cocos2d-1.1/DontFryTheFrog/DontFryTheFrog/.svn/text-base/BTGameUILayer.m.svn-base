...
@synthesize pauseButton, gustButton, lifeCounter;
...
- (id) init {
    if ((self = [super init])){
        ...
        gustButton = [CCMenuItemSprite itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"gustbutton.png"] selectedSprite:[CCSprite spriteWithSpriteFrameName:@"gustbutton.png"] block:^(id sender){
            [(BTGameScene *)[self parent] activateGustOfWind];
        }];
        gustButton.position = ccp(459, 21);
        gustButton.visible = NO;
        gustButton.isEnabled = NO;
        
        lifeCounter = [[[BTLifeCounter alloc] init] autorelease];
        [self addChild:lifeCounter z:2];
        
        CCMenu *menu = [CCMenu menuWithItems:pauseButton, gustButton, nil];
