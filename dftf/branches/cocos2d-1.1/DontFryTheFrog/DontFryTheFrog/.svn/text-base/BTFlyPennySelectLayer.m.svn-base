...
- (void) displayJar:(int) jarLevel {
    ...
    for (int i = 1; i <= 5; i++){
        NSString *unpressed = [NSString stringWithFormat:@"FP-size-%d-unpressed.png", i];
        NSString *pressed = [NSString stringWithFormat:@"FP-size-%d-pressed.png", i];
        if (jarLevel == i){
            unpressed = pressed;
        }
...
//                CCLabelTTF *processingLabel = [CCLabelTTF labelWithString:@"Please wait..." fontName:@"soupofjustice" fontSize:64];
//                processingLabel.position = ccp(240, 160);
//                [self addChild:processingLabel z:3 tag:kOverlay];
