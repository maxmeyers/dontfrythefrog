...
- (id) initWithLastScore:(int) score AllTimeHighScoreBox:(BTHighScoreBox *) allTime WeeklyHighScoreBox:(BTHighScoreBox *) weekly {
    ...
    if (self) {        
        ...
        if ([GKLocalPlayer localPlayer].isAuthenticated && [internetConnection currentReachabilityStatus] != NotReachable){
            ...
            BTGameScene *scene =  (BTGameScene *)[(BTResultsLayer *)[self parent] parent];
...
- (void) update:(ccTime) dt {
    if (drState == kDoesNotHavePreviousScores){
        ...
        if ([allTimeHighScoreBox state] == kLoadingFinished && [weeklyHighScoreBox state] == kLoadingFinished){
            if ([[self parent] isMemberOfClass:[BTResultsLayer class]] && [[[self parent] parent] isMemberOfClass:[BTGameScene class]]){
                // Steal important info from scene
                BTGameScene *scene =  (BTGameScene *)[(BTResultsLayer *)[self parent] parent];
...
- (void) setScores:(NSArray *)array TimeScope:(GKLeaderboardTimeScope)scope {
    ...
    if (allTimePrevious != -1 && weeklyPrevious != -1){
        ...
        if ([[[self parent] parent] isMemberOfClass:[BTGameScene class]])
            [BTGameCenterManager reportScore:lastScore forCategory:@"1000"];
    }
}
