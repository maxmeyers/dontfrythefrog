...
- (void) bugFriedOfClass:(Class)c {
    ...
    if ([c isSubclassOfClass:NSClassFromString(bugClass)] && [otherClass isEqualToString:@"Fries"]){
        [self resetProgress];
    }
}
...
- (void) bugEatenOfClass:(Class)c {
    ...
    if ([c isSubclassOfClass:NSClassFromString(bugClass)] && [otherClass isEqualToString:@"Eats"]){
        [self resetProgress];
    }
}
